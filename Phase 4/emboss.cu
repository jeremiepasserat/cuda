...
__global__ void emboss ( unsigned char * data,   unsigned char * out, std::size_t cols, std::size_t rows) {
  ...
  if ( i > 0 && i < (cols - 1) && j > 0 && j < (rows - 1)) {
    ...
    for (auto c = 0; c < 3; ++c){
      ...
      auto gu =     data[((j - 1) * cols + i - 1) * 3 + c] * -18 + data[((j - 1) * cols + i + 1) * 3 + c] * 0
      +     data[( j      * cols + i - 1) * 3 + c] * -9 +     data[( j      * cols + i + 1) * 3 + c] * 9
      +     data[((j + 1) * cols + i - 1) * 3 + c] * 0  +     data[((j + 1) * cols + i + 1) * 3 + c] * 18
      +     data[(( j - 1) * cols + i) * 3 + c] * -9   +  9 *   data[( j      * cols + i) * 3 + c]
      +     data[(( j + 1) * cols + i) * 3 + c] * 9;
...
int main()
{
  ...
  emboss<<< be, t, 0, streams[ 0 ] >>>( rgb_d, out, cols, rows  / 2 + 2);
  emboss<<< be, t, 0, streams[ 1 ] >>>( rgb_d+size/2, out+size/2, cols, rows / 2);
  ...
  cv::imwrite( "outEmboss.jpg", m_out );
  cudaFree( rgb_d);
  //cudaFree( g_d);
  cudaFree ( out);
