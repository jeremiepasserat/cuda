...
__global__ void mike ( unsigned char * data,   unsigned char * out, std::size_t cols, std::size_t rows) {
  ...
  if ( i > 0 && i < (cols - 1) && j > 0 && j < (rows - 1)) {
    ...
    for (auto c = 0; c < 3; ++c){
       ...
       auto gu =     data[((j - 1) * cols + i - 1) * 3 + c] * 2 + data[((j - 1) * cols + i + 1) * 3 + c]
       +     data[( j      * cols + i - 1) * 3 + c] * 22 +     data[( j      * cols + i + 1) * 3 + c] * -22
       +     data[((j + 1) * cols + i - 1) * 3 + c]  +     data[((j + 1) * cols + i + 1) * 3 + c] * -2
       +     data[(( j - 1) * cols + i) * 3 + c] * 22   +  2 *   data[( j      * cols + i) * 3 + c]
       +     data[(( j + 1) * cols + i) * 3 + c] * -22;
...
//  cudaHostRegister(g.data(), size, cudaHostRegisterDefault);
  ...
  mike<<< be, t, 0, streams[ 0 ] >>>( rgb_d, out, cols, rows / 2 + 2);
  mike<<< be, t, 0, streams[ 1 ] >>>( rgb_d+size/2, out+size/2, cols, rows / 2);
