...
int main() {
  ...
  for(j = 1 ; j < height - 1 ; ++j) {
    ...
    h = fmin(data[((j - 1) * width + i - 1) * 3 + c], data[((j - 1) * width + i + 1) * 3 + c]);
    h = fmin(h, data[(( j - 1) * width + i) * 3 + c]);
    m = fmin(data[( j * width + i - 1) * 3 + c],  data[( j * width + i + 1) * 3 + c]);
    m = fmin(m, data[( j * width + i) * 3 + c]);
