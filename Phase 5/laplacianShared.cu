...
__global__ void laplacianShared ( unsigned char * data,   unsigned char * out, std::size_t w, std::size_t h) {
  ...
  if( i < w && j < h ) {
    ...
    if( li > 0 && li < (blockDim.x - 1) && lj > 0 && lj < (blockDim.y - 1) )
    {
      for (auto c = 0; c < 3; ++c){
          ...
          auto gu =     sh[((lj - 1) * ww + li - 1) * 3 + c] * 0 +     sh[((lj - 1) * ww + li + 1) * 3 + c] * 0
      +     sh[( lj      * ww + li - 1) * 3 + c] * -1 +     sh[( lj      * ww + li + 1) * 3 + c] * -1
      +     sh[((lj + 1) * ww + li - 1) * 3 + c] * 0 +     sh[((lj + 1) * ww + li + 1) * 3 + c] * 0
      +     sh[(( lj - 1) * ww + li) * 3 + c] * -1     +     sh[( lj      * ww + li) * 3 + c] * 4
      +     sh[(( lj + 1) * ww + li) * 3 + c] * -1;
...
//  cudaHostRegister(g.data(), size, cudaHostRegisterDefault);
  ...
  laplacianShared<<< be, t, 3 * t.x * t.y, streams[ 0 ] >>>( rgb_d, out, cols, rows / 2 + 2);
  laplacianShared<<< be, t, 3 * t.x * t.y, streams[ 1 ] >>>( rgb_d+size/2, out+size/2, cols, rows / 2);
  ...
  cv::imwrite( "outLaplacianShared.jpg", m_out );
  cudaFree( rgb_d);
  //cudaFree( g_d);
  cudaFree ( out);
