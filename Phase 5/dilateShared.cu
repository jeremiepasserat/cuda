  ...
  if( i < w + 2 && j < h + 2 ) {
    ...
    if( li > 0 && li < (blockDim.x - 1) && lj > 0 && lj < (blockDim.y - 1) )
    {
      for (auto c = 0; c < 3; ++c){
        ...
        auto hu = umin(sh[((lj - 1) * ww + li - 1) * 3 + c], sh[((lj - 1) * ww + li + 1) * 3 + c]);
        hu = umin(hu, sh[(( lj - 1) * ww + li) * 3 + c]);
        auto mu = umin(sh[( lj * ww + li - 1) * 3 + c],  sh[( lj * ww + li + 1) * 3 + c]);
        mu = umin(mu, sh[( lj * ww + li) * 3 + c]);
        auto bu = umin(sh[((lj + 1) * ww + li - 1) * 3 + c], sh[((lj + 1) * ww + li + 1) * 3 + c]);
        bu = umin(bu, sh[((lj + 1) * ww + li) * 3 + c]);
...
//  cudaHostRegister(g.data(), size, cudaHostRegisterDefault);
  ...
  dilateShared<<< be, t, 3 * t.x * t.y, streams[ 0 ] >>>( rgb_d, out, cols, rows/2);
  dilateShared<<< be, t, 3 * t.x * t.y, streams[ 1 ] >>>( rgb_d+size/2, out+size/2, cols, rows/2);
  ...
  cv::imwrite( "outDilateShared.jpg", m_out );
  cudaFree( rgb_d);
  //cudaFree( g_d);
  cudaFree ( out);
